...
export function handleWeb3Error(error: any): never {
    ...
    if (
        ...
        throw new BadRequestError(
            "Insufficient funds: Please ensure your wallet has enough BASE tokens to cover the transaction gas fees",
        );
    }
    ...
    if (
        ...
        error?.name === "UNPREDICTABLE_GAS_LIMIT" ||
    ...
    if (
        errorDetails.code === "ACTION_REJECTED" ||
    ...
    throw new InternalServerError(
        errorDetails.message ||
            "An unexpected error occurred during the blockchain operation",
    );
}
