...
const fileFilter = (
    ...
    if (allowedImageTypes.includes(file.mimetype)) {
        ...
        cb(
            new BadRequestError(
                `Unsupported ${fileType} type. ${supportedTypes}`,
            ),
        );
    }
};
...
export const uploadMiddleware = function (
    ...
    return (req: Request, res: Response, next: NextFunction) => {
        ...
        case UploadType.Array:
            multerMiddleware = multer.array(
                    nameOrFields as string,
                    maxCount,
            );
            break;
        case UploadType.Fields:
            multerMiddleware = multer.fields(
                    nameOrFields as Multer.Field[],
            );
            break;
        default:
        ...
        multerMiddleware(req, res, (err) => {
            ...
            console.log(
                "Multer middleware completed, proceeding to next middleware",
            );
            next();
        });
    };
