...
export type ValidRole = 'host' | 'coHost' | 'guest' | 'speaker' | 'listener' | 'bot';
 ...
 */
export const generateCallToken = async (options: GenerateTokenOptions): Promise<string> => {
  const { apiKey, roomId, role, permissions, metadata } = options;
  
  // Validate role
  const validRoles: ValidRole[] = ['host', 'coHost', 'guest', 'speaker', 'listener', 'bot'];
  ...
  const requiredPermissionFields: (keyof TokenPermissions)[] = [
    'admin', 'canConsume', 'canProduce', 'canProduceSources',
    'canRecvData', 'canSendData', 'canUpdateMetadata'
  ];
  ...
  for (const field of requiredPermissionFields) {
    if (field === 'canProduceSources') {
      ...
      if (cam === undefined || mic === undefined || screen === undefined) {
        throw new Error('All canProduceSources fields (cam, mic, screen) are required');
      }
