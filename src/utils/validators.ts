...
class Validator {
    ...
    static uploadFIleValidatorToGetExtension({
        mimeType,
        fileName,
        fileSizeInBytes,
        durationInSeconds,
    }: IFileDataType): string {
        const missingFields = [];
        if (!mimeType) missingFields.push("mimeType");
        if (!fileName) missingFields.push("fileName");
        if (!fileSizeInBytes) missingFields.push("fileSizeInBytes");
        if (!durationInSeconds && mimeType.includes("video/"))
            missingFields.push("durationInSeconds");
        if (missingFields.length > 0) {
            throw new BadRequestError(
                `Please provide ${missingFields.join(", ")} parameter(s)`,
            );
        }
        ...
        if (!isValid) {
            throw new BadRequestError(
                "Invalid file name, file name must have an extension",
            );
        }

        const isMimeTypeValid = this.isValidMimeType(mimeType);
        ...
        if (isValid && mimeType.includes("video/")) {
            if ((durationInSeconds as number) > 300) {
                throw new BadRequestError(
                    "Video duration exceeds the 5 minute limit",
                );
            }
        }
    ...
    static validateMultipleFilesForUpload(
        files: IFileDataType[],
        uploadClass: AWSUploadType,
    ): { processable: ProcessableFile[]; unprocessible: UnprocessableFile[] } {
        ...
        files.forEach((file) => {
            ...
            if (!file.durationInSeconds && file.mimeType.includes("video/"))
                missingFields.push("durationInSeconds");
            ...
            const { isValid, extension } = Validator.isValidFilename(
                file.fileName,
            );
            if (!isValid) {
                unprocessible.push({
                    file,
                    reason: "Invalid file name, must have an extension",
                });
                return;
            }
            ...
            if (
                ...
                (file.durationInSeconds as number) > 300
            ) {
                unprocessible.push({
                    file,
                    reason: "Video duration exceeds the 5 minute limit",
                });
                return;
            }
            ...
            if (fileSizeInMB > 50) {
                unprocessible.push({
                    file,
                    reason: "File size exceeds the 50MB limit",
                });
                return;
            }
