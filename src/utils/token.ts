...
class TokenCacheUtil {
    ...
    static async saveTokenToCacheList({
        key,
        token,
        expiry,
    }: SaveTokenToCache) {
    ...
    static async saveAuthTokenToCache({
        key,
        token,
        expiry,
    }: SaveTokenToCache) {
...
class AuthUtil {
    static getSecretKeyForTokenType(type: ENCRYPTEDTOKEN): {
        secretKey: string;
        expiry: number;
    } {
        switch (type) {
    ...
    static createWalletHash(walletAddress: string): {
        hash: string;
        timestamp: number;
        nonce: string;
    } {
    ...
    static async generateTokenWithHash(info: GenerateTokenData) {
        ...
        const {
            ...
            timestamp,
            nonce,
        } = this.createWalletHash(user.walletAddress);
