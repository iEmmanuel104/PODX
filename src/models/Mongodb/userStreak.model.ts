...
UserStreakSchema.virtual("weeklyActivity").get(function () {
    ...
    return this.callActivities.filter((activity) => activity.date >= weekAgo)
        .length;
});
...
UserStreakSchema.pre("save", function (next) {
    if (this.isModified("callActivities")) {
        ...
        this.stats.totalCallDuration = durations.reduce(
            (sum, dur) => sum + dur,
            0,
        );
        this.stats.averageCallDuration = Math.round(
            this.stats.totalCallDuration / durations.length,
        );
