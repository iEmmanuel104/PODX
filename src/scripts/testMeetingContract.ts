...
if (!process.env.FACTORY_ADDRESS || !process.env.PRIVATE_KEY || !process.env.COINBASE_API_KEY) {
    console.error('Missing required environment variables:')
    console.error('FACTORY_ADDRESS:', process.env.FACTORY_ADDRESS ? '✅' : '❌')
    console.error('PRIVATE_KEY:', process.env.PRIVATE_KEY ? '✅' : '❌')
    console.error('COINBASE_API_KEY:', process.env.COINBASE_API_KEY ? '✅' : '❌')
    process.exit(1)
}
...
async function main() {
    try {
        ...
        const metadataURL = "ipfs://bafkreigh3r7sfkclis5v7n6ovwyvmd2eanhojwpq3uwy2idgmxlg6feky4"
