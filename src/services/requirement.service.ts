...
export default class RequirementService {
    ...
    static async getParticipantDetails(callId: string): Promise<ParticipantTime[]> {
        ...
        if (call.custom?.events && call.custom.events.length > 0) {
            ...
            const eventsByUser = call.custom.events.reduce((acc: Record<string, ParticipantEvent[]>, event: DbEvent) => {
            ...
            participantTimes.push(...call.members.map((member: any) => ({
                userId: member.userId.toString(),
                timeSpentMinutes: call.duration ? Math.floor(call.duration / 60) : 0, // Convert seconds to minutes
            })));
        }
    ...
    private static async getCallDetails(callId: string): Promise<CallDetails> {
        ...
        const scheduledDuration = call.scheduledTime 
            ? Math.ceil((new Date(call.scheduledTime).getTime() - startTime) / (60 * 1000))
            : sessionTimeMinutes;
        ...
        if (durationReq) {
            if (durationReq.type === 'absolute') {
                ...
                requiredDurationMinutes = Math.ceil((percentage / 100) * scheduledDuration);
            }
        } else {
    ...
    static async checkAllRequirements(callId: string): Promise<RequirementCheckResult> {
        try {
                ...
                .map((member: any) => {
                    ...
                    if (call.custom?.events) {
                        call.custom.events.forEach((event: any) => {
                            if (event) {
                                const eventUserId = event.userId && typeof event.userId !== 'string' 
                                    ? event.userId.toString() 
                                    : event.userId;
                    ...
                    const timeSpent = this.calculateParticipantDuration(convertedEvents, userId);
            ...
            const eligibleParticipants = participantDetails.filter((p: ParticipantEligibility) => p.eligible).length;
