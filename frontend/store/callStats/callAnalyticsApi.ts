...
import { ApiResponse, CallStatsResponse, DetailedCallStatsResponse, LeaderboardResponse, QueryParams } from "./types";
...
export const callAnalyticsApiSlice = api.injectEndpoints({
    endpoints: builder => ({
        ...
        listCallDetails: builder.query<ApiResponse<{calls: any[]}>, string>({
            ...
            providesTags: (_result, _error, callId) => [createTag({
                id: callId,
...
export const {useListCallStatsQuery, useListDetailedCallStatsQuery, useListUserCallsQuery, useListCallDetailsQuery, useListLeaderboardQuery } = callAnalyticsApiSlice;