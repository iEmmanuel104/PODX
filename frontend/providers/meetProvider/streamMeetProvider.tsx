...
export const StreamMeetProvider = memo<StreamMeetProviderProps>(({ meetingId, children, language }) => {
    ...
    const connectChatClient = useCallback(
        async (token: string) => {
            if (!chatClientRef.current) {
                chatClientRef.current = await connectionPool.getChatClient(STREAM_API_KEY as string);
            }
    ...
    return (
        <ErrorBoundary fallback={<div>Something went wrong with the stream connection. Please try again.</div>}>
            <div className="w-full h-full">
