...
const WalletOperations: React.FC<WalletOperationsProps> = ({ state, setState, user, onWithdrawClick, onWarningConfirm }) => {
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <div>
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="bg-[#6032F6] rounded-full px-2 py-0.5 text-xs">{displayBalance} ETH</div>
                        </div>
                    </div>
                </div>
            </div>

            <DropdownMenuItem onSelect={onWithdrawClick} className="flex items-center px-3 py-2 cursor-pointer">
                <RefreshCcw className="mr-2 h-4 w-4" />
                <span>Withdraw funds</span>
            </DropdownMenuItem>

            <DropdownMenuItem className="flex items-center px-3 py-2 cursor-pointer" onSelect={handleExport}>
                ...
                <Dialog open={state.isWarningOpen} onOpenChange={(open) => setState((prev) => ({ ...prev, isWarningOpen: open }))}>
                    <DialogContent className="bg-[#1d1d1d] border-0 text-white max-w-[320px] sm:rounded-3xl rounded-2xl">
                        ...
                        <DialogFooter className="sm:justify-center">
                            <Button className="w-full bg-[#6032f6] hover:bg-[#4C28C4] text-white rounded-xl py-6" onClick={onWarningConfirm}>
                                I understand.
                            </Button>
                        </DialogFooter>
                    </DialogContent>
